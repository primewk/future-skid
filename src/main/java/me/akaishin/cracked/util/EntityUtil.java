...
import me.akaishin.cracked.features.modules.player.Blink;
...
class EntityUtil implements Util {
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || (entity instanceof EntityPlayer && Cracked.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (player.isInvisible() && !HUD.getInstance().tRadarInv.getValue()) {
                continue;
            }
            ...
            output.put(healthSB + " " + (Cracked.friendManager.isFriend(player) ? "\u00A7b" : "\u00A7r") + player.getName() + " " + distanceSB + " " + "\u00A7f" + Cracked.totemPopManager.getTotemPopString(player) + Cracked.potionManager.getTextRadarPotion(player), (int) EntityUtil.mc.player.getDistance(player));
