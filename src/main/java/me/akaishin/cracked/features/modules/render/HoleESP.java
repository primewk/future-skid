...
import me.akaishin.cracked.util.RotationUtil;
...
public class HoleESP
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        for (BlockPos pos : Cracked.holeManager.getSortedHoles()) {
            ...
            if (pos.equals(new BlockPos(HoleESP.mc.player.posX, HoleESP.mc.player.posY, HoleESP.mc.player.posZ)) && !this.ownHole.getValue().booleanValue() /*|| !RotationUtil.isInFov(pos)*/)
                continue;
            if (this.safeColor.getValue().booleanValue() && Cracked.holeManager.isSafe(pos)) {
    ...
    public void onSettingChange(ClientEvent event) {
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    Cracked.holeManager.settingChanged();
                }
            }
        }
