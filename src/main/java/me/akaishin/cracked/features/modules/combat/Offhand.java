...
import me.akaishin.cracked.mixin.mixins.accessors.IContainer;
import me.akaishin.cracked.mixin.mixins.accessors.ISPacketSetSlot;
...
public class Offhand
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (this.noOffhandGC.getValue().booleanValue() && !Offhand.fullNullCheck() && Offhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && Offhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && Offhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
            ...
            if (event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock) {
                ...
                if (packet2.getHand() == EnumHand.MAIN_HAND && !CrystalAura.placedPos.contains(packet2.getPos())) {
                    if (this.timer.passedMs(this.timeout.getValue().intValue())) {
                        Offhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !Cracked.friendManager.isFriend(e) && Offhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }
