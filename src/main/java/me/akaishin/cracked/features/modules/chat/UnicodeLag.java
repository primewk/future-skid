...
import me.akaishin.cracked.util.unicodelag.Timer;
...
public class UnicodeLag
        extends Module {
	private final Setting<ModeSetting> messageMode = this.register(new Setting<ModeSetting>("MessageMode", ModeSetting.DM));
	...
	public UnicodeLag() {
        super("UnicodeLag", "Screams chinese words on your enemy", Module.Category.CHAT, true, false, false);
    }
    ...
    public void onChat(ServerChatEvent event) {
        if (event.getMessage().contains(lagText)) {
            if (!event.isCanceled()) {
                ...
                if (this.cancelMessage.getValue()) Command.sendMessage("Canceled you from seeing lag text.");
            }
        }
    }
    ...
    public void onTotemPop(TotemPopEvent event) {
		if (!timer.passed((long) (this.delay.getValue()))) return;
	...
	public static enum ModeSetting {
		DM,
		Global;
	}
}