...
public class Speed
    ...
    public Setting<Boolean> speedgeneral = this.register(new Setting<Boolean>("ElytraStrafe", true));
    ...
    private boolean shouldReturn() {
        return Cracked.moduleManager.isModuleEnabled("Freecam") || Cracked.moduleManager.isModuleEnabled("PacketFly") || Cracked.moduleManager.isModuleEnabled("ElytraFlight") || Cracked.moduleManager.isModuleEnabled("Strafe") || Cracked.moduleManager.isModuleEnabled("Flight") || Cracked.moduleManager.isModuleEnabled("SpeedNew");
    }
        ...
        if (EntityUtil.isEntityMoving((Entity)Speed.mc.player) && !Speed.mc.player.collidedHorizontally && !BlockUtil.isBlockAboveEntitySolid((Entity)Speed.mc.player) && BlockUtil.isBlockBelowEntitySolid((Entity)Speed.mc.player)) {
            ...
            if (Speed.mc.player.posY >= this.startY + this.bounceHeight) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    Cracked.timerManager.setTimer(1.0f);
                }
            }
            if (Speed.mc.player.posY == this.startY) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    if (rnd) {
                        Cracked.timerManager.setTimer(1.3f);
                    } else {
                        Cracked.timerManager.setTimer(1.0f);
                    }
                }
