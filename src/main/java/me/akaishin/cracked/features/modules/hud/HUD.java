...
import me.akaishin.cracked.features.modules.misc.ToolTips;
...
public class HUD
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "Cracked", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public Setting<Boolean> potionst = this.register(new Setting<Boolean>("Potionst", true));
    ...
    public void onUpdate() {
        for (final Module module : Cracked.moduleManager.sortedModules) {
            if (module.isDisabled() && module.arrayListOffset == 0.0f) {
                module.sliding = true;
            }
        }
        if (this.timer.passedMs(textRadarUpdates.getValue())) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case CRACKED: {
                this.renderer.drawString("Cracked " + (this.modeVer.getValue() ? Cracked.MODVER : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Cracked.moduleManager.alphabeticallySortedModules.size() : Cracked.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Cracked.moduleManager.alphabeticallySortedModules.get(k) : Cracked.moduleManager.sortedModules.get(k);
                    ...
                    final Color moduleColor = Cracked.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Cracked.moduleManager.alphabeticallySortedModules.size() : Cracked.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Cracked.moduleManager.alphabeticallySortedModules.get(Cracked.moduleManager.alphabeticallySortedModules.size() - 1 - k) : Cracked.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + ChatFormatting.GRAY + ((module.getDisplayInfo() != null) ? (" [" + ChatFormatting.WHITE + module.getDisplayInfo() + ChatFormatting.GRAY + "]") : "");
                    final Color moduleColor = Cracked.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + ChatFormatting.WHITE + Cracked.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Cracked.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? Cracked.potionManager.getPotionString(effect) : Cracked.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + ChatFormatting.WHITE + Cracked.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + ChatFormatting.WHITE + Cracked.serverManager.getTPS();
            ...
            final String text = grayString + "Ping " + ChatFormatting.WHITE + Cracked.serverManager.getPing() + (this.MS.getValue() ? "ms" : "");//remove ping bypass
            ...
            final String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            final String text = grayString + "Ping " + ChatFormatting.WHITE + Cracked.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue()) {
            Cracked.notificationManager.handleNotifications(height - (k + 16));
        } else {
            Cracked.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        final String text4 = (this.direction.getValue() ? (Cracked.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Cracked " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (Cracked.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? ChatFormatting.GRAY : ChatFormatting.RED) + "Server not responding: " + MathUtil.round(Cracked.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
