...
public class Killaura
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                Cracked.rotationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int wait = this.oneDotEight.getValue().booleanValue() ? 0 : (wait = (int) ((float) DamageUtil.getCooldownByWeapon(Killaura.mc.player) * (this.tps.getValue() != false ? Cracked.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue().booleanValue()) {
            Cracked.positionManager.setPositionPacket(Killaura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double) target.getEyeHeight(), Killaura.target.posZ, true, true, this.lagBack.getValue() == false);
        }
