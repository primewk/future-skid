package me.akaishin.cracked.features.modules.render.tracersold;
...
import me.akaishin.cracked.features.modules.render.tracersold.util.EntityUtil;
import me.akaishin.cracked.features.modules.render.tracersold.util.MathUtil;
...
public class TracerOld
    ...
    public void onRender3D(Render3DEvent event) {
        if (TracerOld.fullNullCheck()) {
            return;
        }
        GlStateManager.pushMatrix();
        TracerOld.mc.world.loadedEntityList.stream().filter(EntityUtil::isLiving).filter(entity -> entity instanceof EntityPlayer ? this.players.getValue().booleanValue() && TracerOld.mc.player != entity : (EntityUtil.isPassive(entity) ? this.animals.getValue().booleanValue() : this.mobs.getValue().booleanValue())).filter(entity -> TracerOld.mc.player.getDistanceSq(entity) < MathUtil.square(this.distance.getValue().intValue())).filter(entity -> this.invisibles.getValue() != false || !entity.isInvisible()).forEach(entity -> {
    ...
    public double[] interpolate(Entity entity) {
        double posX = this.interpolate(entity.posX, entity.lastTickPosX) - TracerOld.mc.getRenderManager().renderPosX;
        double posY = this.interpolate(entity.posY, entity.lastTickPosY) - TracerOld.mc.getRenderManager().renderPosY;
        double posZ = this.interpolate(entity.posZ, entity.lastTickPosZ) - TracerOld.mc.getRenderManager().renderPosZ;
    ...
    public void drawLine(double posx, double posy, double posz, double up, float red, float green, float blue, float opacity) {
        Vec3d eyes = new Vec3d(0.0, 0.0, 1.0).rotatePitch(-((float) Math.toRadians(TracerOld.mc.player.rotationPitch))).rotateYaw(-((float) Math.toRadians(TracerOld.mc.player.rotationYaw)));
        if (!this.drawFromSky.getValue().booleanValue()) {
            this.drawLineFromPosToPos(eyes.x, eyes.y + (double) TracerOld.mc.player.getEyeHeight(), eyes.z, posx, posy, posz, up, red, green, blue, opacity);
        } else {
    ...
    public void drawLineFromPosToPos(double posx, double posy, double posz, double posx2, double posy2, double posz2, double up, float red, float green, float blue, float opacity) {
        ...
        GL11.glLoadIdentity();
        TracerOld.mc.entityRenderer.orientCamera(mc.getRenderPartialTicks());
        GL11.glBegin(1);
    ...
    public float[] getColorByDistance(Entity entity) {
        if (entity instanceof EntityPlayer && Cracked.friendManager.isFriend(entity.getName())) {
            return new float[]{0.0f, 0.5f, 1.0f, 1.0f};
        }
        AutoCrystal autoCrystal = Cracked.moduleManager.getModuleByClass(AutoCrystal.class);
        Color col = new Color(Color.HSBtoRGB((float) (Math.max(0.0, Math.min(TracerOld.mc.player.getDistanceSq(entity), this.crystalCheck.getValue() != false ? (double) (autoCrystal.placeRange.getValue().floatValue() * autoCrystal.placeRange.getValue().floatValue()) : 2500.0) / (double) (this.crystalCheck.getValue() != false ? autoCrystal.placeRange.getValue().floatValue() * autoCrystal.placeRange.getValue().floatValue() : 2500.0f)) / 3.0), 1.0f, 0.8f) | 0xFF000000);
