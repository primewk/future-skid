...
class NoSoundLag
    ...
    public void onPacketReceived(PacketEvent.Receive event) {
        if (event != null && event.getPacket() != null && NoSoundLag.mc.player != null && NoSoundLag.mc.world != null && event.getPacket() instanceof SPacketSoundEffect) {
            ...
            if (this.crystals.getValue() && packet.getCategory() == SoundCategory.BLOCKS && packet.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE && (CrystalAura.getInstance().isOff() || !CrystalAura.getInstance().sound.getValue() && CrystalAura.getInstance().threadMode.getValue() != CrystalAura.ThreadMode.SOUND)) {
