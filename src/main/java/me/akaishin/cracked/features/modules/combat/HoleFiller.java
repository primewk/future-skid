...
public class HoleFiller
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Cracked.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Cracked.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Cracked.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Cracked.holeManager.getMidSafety());
            }
        }
        object = Cracked.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Cracked.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Cracked.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
