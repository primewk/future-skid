...
import me.akaishin.cracked.event.events.ProcessRightClickBlockEvent;
...
import me.akaishin.cracked.features.setting.EnumConverter;
...
public class OffhandRewrite
        ...
        if (Keyboard.getEventKeyState() && this.switchmode.getValue().booleanValue() && this.SwitchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchval < this.maxSwitch.getValue()) {
                ...
                if (this.debug.getValue().booleanValue()) {
                    TextComponentString textComponentString = new TextComponentString(Cracked.commandManager.getClientMessage() + " \u00a7r\u00a7aSwitched offhand to " + newMode.toString());
            ...
            if (this.debug.getValue().booleanValue()) {
                new TextComponentString(Cracked.commandManager.getClientMessage() + " \u00a7r\u00a7aReached switch limit interval");
            }
        }
    }
    ...
    public void doSwitch() {
        ...
        } else if (this.rightGap.getValue().booleanValue() && OffhandRewrite.mc.player.getHealth() > this.switchHp.getValue().floatValue() && OffhandRewrite.mc.gameSettings.keyBindUseItem.isKeyDown() && OffhandRewrite.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && !(OffhandRewrite.mc.currentScreen instanceof GuiContainer) && !(OffhandRewrite.mc.currentScreen instanceof GuiChat) && !(OffhandRewrite.mc.currentScreen instanceof CrackedHackGui)) {
            this.currentMode = Mode2.GAPPLES;
        ...
        if (this.crystalCheck.getValue().booleanValue() && this.calcCrystal()) {
            if (this.debug.getValue().booleanValue()) {
                TextComponentString textComponentString = new TextComponentString(Cracked.commandManager.getClientMessage() + " \u00a7r\u00a7aSwitched to totem because lethal crystal");
                Notifications.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((ITextComponent)textComponentString, this.s.length() * 10);
            }
            this.switchedForHealthReason = true;
            this.currentMode = Mode2.TOTEMS;
        ...
        if (this.antiPing.getValue().booleanValue() && ((long)Cracked.serverManager.getPing()) > (long)this.pingvalue.getValue().intValue()) {
            this.setMode(Mode2.TOTEMS);
        }
        if (this.lagSwitch.getValue().booleanValue() && Cracked.serverManager.isServerNotResponding()) {
            this.setMode(Mode2.TOTEMS);
        }
