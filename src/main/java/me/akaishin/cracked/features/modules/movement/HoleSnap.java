...
import me.akaishin.cracked.features.modules.movement.SpeedNew;
import me.akaishin.cracked.features.setting.Setting;
import me.akaishin.cracked.util.EntityUtil;
import me.akaishin.cracked.util.Timer;
import me.akaishin.cracked.util.oyveyutils.HoleUtilSafety;
...
public class HoleSnap
    ...
    public void onTick() {
        ...
        if (this.mode.getValue() == Mode.Instant) {
            blockPos2 = Cracked.holeManager.calcHoles().stream().min(Comparator.comparing(blockPos -> HoleSnap.mc.player.getDistance((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()))).orElse(null);
            if (blockPos2 != null) {
