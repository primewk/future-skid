...
public class PistonAura
    ...
    public void onTick() {
        try {
            ...
            this.debug_stage = 3;
            boolean bl = doTrap = this.trap.getValue() == mode.Force || this.trap.getValue() == mode.Smart && Cracked.holeManager.isSafe(PistonAura.mc.player.getPosition()) && this.b_piston.getY() == PistonAura.mc.player.getPosition().getY() + 1;
            ...
            if (!this.p_crystal) {
                ...
                PistonAura.mc.playerController.updateController();
                CrystalAura.mc.player.connection.sendPacket((Packet) new CPacketPlayerTryUseItemOnBlock(this.b_crystal, EnumFacing.UP, PistonAura.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                this.p_crystal = true;
            }
            this.debug_stage = 9;
            if (!this.p_redstone) {
