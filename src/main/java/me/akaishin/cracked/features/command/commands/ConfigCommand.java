...
public class ConfigCommand
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            ConfigCommand.sendMessage("You`ll find the config files in your gameProfile directory under cracked/config");
            return;
        }
        if (commands.length == 2) {
            if ("list".equals(commands[0])) {
                String configs = "Configs: ";
                File file = new File("Cracked/");
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save": {
                    Cracked.configManager.saveConfig(commands[1]);
                    ConfigCommand.sendMessage("\u00a7aConfig has been saved.");
                    break;
                }
                case "load": {
                    Cracked.moduleManager.onUnload();
                    Cracked.configManager.loadConfig(commands[1]);
                    Cracked.moduleManager.onLoad();
