...
public class ColorUtil {
   ...
   public static int staticRainbow(float offset, Color color) {
      double timer = (double)System.currentTimeMillis() % 1750.0D / 850.0D;
      float[] hsb = new float[3];
      Color.RGBtoHSB(color.getRed(), color.getGreen(), color.getBlue(), hsb);
      float brightness = (float)((double)hsb[2] * Math.abs(((double)offset + timer) % 1.0D - 0.550000011920929D) + 0.44999998807907104D);
   ...
   public String getColorNameFromRgb(int r, int g, int b) {
      ...
      Iterator var7 = colorList.iterator();

      while(var7.hasNext()) {
         ColorUtil.ColorName c = (ColorUtil.ColorName)var7.next();
   ...
   public static class Colors {
      ...
      public static final int AQUA = ColorUtil.toRGBA(0, 255, 0, 255);
