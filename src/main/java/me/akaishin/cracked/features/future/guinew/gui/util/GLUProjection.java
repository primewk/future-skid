...
public final class GLUProjection {
   ...
   public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale, double heightScale) {
      ...
      double pitch = (!(-fv.y > 0.0D) || !(yaw >= 90.0D) || !(yaw < 270.0D)) && (!(fv.y > 0.0D) || yaw >= 90.0D && yaw < 270.0D) ? -Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv))) : Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv)));
   ...
   public GLUProjection.Projection project(double x, double y, double z, GLUProjection.ClampMode clampModeOutside, boolean extrudeInverted) {
      if (this.viewport != null && this.modelview != null && this.projection != null) {
         ...
         if (outsideFrustum) {
            ...
            } else if ((!extrudeInverted || outsideInvertedFrustum) && clampModeOutside != GLUProjection.ClampMode.DIRECT) {
               if (clampModeOutside != GLUProjection.ClampMode.ORTHOGONAL) {
                  return new GLUProjection.Projection(0.0D, 0.0D, GLUProjection.Projection.Type.FAIL);
                  ...
                  if (guiY < 0.0D) {
                     guiY = 0.0D;
                     return new GLUProjection.Projection(guiX, guiY, GLUProjection.Projection.Type.OUTSIDE);
