...
import me.akaishin.cracked.util.GLUProjection;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            Cracked.potionManager.update();
            Cracked.totemPopManager.onUpdate();
            Cracked.inventoryManager.update();
            Cracked.holeManager.update();
            Cracked.safetyManager.onUpdate();
            Cracked.moduleManager.onUpdate();
            Cracked.timerManager.update();
            if (this.timer.passedMs(ClickGui.getInstance().moduleListUpdates.getValue().intValue())) {
                Cracked.moduleManager.sortModules(true);
                Cracked.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && mc.getConnection() != null && EventManager.mc.world != null) {
            String command = "@Server" + "module " + event.getSetting().getFeature().getName() + " set " + event.getSetting().getName() + " " + event.getSetting().getPlannedValue().toString();
    ...
    public void onTickLowest(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            CrystalAura.getInstance().postTick();
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        Cracked.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        Cracked.moduleManager.onLogout();
        Cracked.totemPopManager.onLogout();
        Cracked.potionManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            Cracked.speedManager.updateValues();
            Cracked.rotationManager.updateRotations();
            Cracked.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            Cracked.rotationManager.restoreRotations();
            Cracked.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        Cracked.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                Cracked.totemPopManager.onTotemPop(player);
                Cracked.potionManager.onTotemPop(player);
            }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        EventManager.mc.profiler.startSection("Cracked");
        ...
        Cracked.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            Cracked.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            Cracked.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    Cracked.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
