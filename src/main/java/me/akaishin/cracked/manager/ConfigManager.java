...
public class ConfigManager
    ...
    public String config = "Cracked/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                Cracked.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Cracked.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Cracked").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("Cracked/" + name + "/")) ? "Cracked/" + name + "/" : "Cracked/config/";
    ...
    public void saveConfig(String name) {
        ...
        this.config = "Cracked/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        Cracked.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Cracked/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Cracked", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Cracked.moduleManager.modules);
        this.features.add(Cracked.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Cracked.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Cracked.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
